...
def create_skewed_sequences(r1s_end:float, r2s_end:float, theta1s_end:float, theta2s_end:float, hs_end:float, output_dir:str, template_3dimage_path:str, template_mask_path:str, num_frames:int, zero_outside_mask:bool):
    r1s_start = 1.0
    r2s_start = 1.0
    theta1s_start = 0.
    theta2s_start = 0.
    ...
    volume_skewer =  VolumeSkewer(zero_outside_mask=zero_outside_mask, warping_borders_pad='zeros', image_warping_interp_mode='bilinear', mask_warping_interp_mode='nearest')
    ...
    maximal_flow_for_mask = volume_skewer.flow_for_mask

    for n, (ratio, r1, r2, theta1, theta2, h) in enumerate(zip(ratios, r1s, r2s, theta1s, theta2s, hs)):
        volume_skewer.scaled_flow_for_image      = maximal_flow_for_image * ratio
        volume_skewer.scaled_flow_for_mask       = maximal_flow_for_mask * ratio
        if n  == 0:
    ...
    return template_synthetic_image_path, unlabeled_synthetic_image_path, template_synthetic_mask_path, unlabeled_synthetic_mask_path, synthetic_flow_path
